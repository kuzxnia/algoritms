...
def sum_double(a, b):
    result = a + b
    return result if a != b else result * 2
...
def diff21(n):
    result = abs(21 - n)
    return result * 2 if n > 21 else result
...
def parrot_trouble(talking, hour):
    return talking and hour not in range(7, 20)
...
def not_string(string):
    return string if str(string).startswith("not") else "not " + string
