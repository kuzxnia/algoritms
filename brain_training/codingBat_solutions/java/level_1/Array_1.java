...
public class Array_1 {
    ...
    public int[] rotateLeft3(int[] nums) {
        int[] tab = Arrays.copyOfRange(nums, 1, 4);
        tab[2] = nums[0];
        return tab;
    }
     ...
     */
    public int[] reverse3(int[] validData) {
    ...
    public int start1(int[] a, int[] b) {
        int result = 0;
        result += (a.length > 0 && a[0] == 1)? 1 : 0;
        result += (b.length > 0 && b[0] == 1)? 1 : 0;
        return result;
    }
    ...
    public int maxTriple(int[] nums) {
        int a = 0;
        a = Math.max(nums[0], nums[nums.length-1]);
        a = Math.max(a, nums[nums.length/2]);
        return a;
    }
    ...
    public boolean unlucky1(int[] n) {
        if(n.length < 2)
            return false;
                ...
                (n[n.length-2] == 1 && n[n.length-1] == 3) ||
                (n.length >= 3 && n[1] == 1 && n[2] == 3))
            return true;
        else
            return false;
    ...
    public int[] front11(int[] a, int[] b) {
        ...
        if(a.length > 0) c.add(a[0]);
        if(b.length > 0) c.add(b[0]);
